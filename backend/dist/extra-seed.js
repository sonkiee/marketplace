...
exports.seedMoreProducts = seedMoreProducts;
...
async function upsertBrand(name, description) {
    ...
    const [row] = await db_1.db
        ...
        .returning({ id: schema_1.brands.id, slug: schema_1.brands.slug });
    return row;
}
async function upsertCategory(name, description) {
    ...
    const [row] = await db_1.db
        ...
        .values({
        name: name, // must match your categoryNameEnum
        slug,
        description: description ?? null,
        isActive: true,
    })
        ...
        .returning({ id: schema_1.categories.id, slug: schema_1.categories.slug });
    return row;
}
...
async function insertVariants(values) {
    if (!values.length)
        return;
    await db_1.db
        ...
        sku: v.sku,
        condition: v.condition, // must match productConditionEnum
...
async function seedMoreProducts() {
    ...
    const samsung = await upsertBrand("Samsung", "Samsung devices and accessories");
    ...
    const catAccessories = await upsertCategory("accessories", "Phone accessories and parts");
    // --------------------
    // iPhone products (3–5)
    // --------------------
